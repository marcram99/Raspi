...
if new_val != data['light_mode']:
    last_change = str(datetime.now())
    warning_level = 0
    light_mode = new_val
    with open(data_file,'w') as json_file:
        data = {"light_mode": new_val,
                "last_change": now, 
                "warning_level": 0
                }
        json.dump(data, json_file)
    write_2_log(f"{now}_light mode passed to: {new_val}\n")
    ...
    if new_val == 'light':
        ...
        if warning_level < 1:
            if time_delta > timedelta(minutes=1):
                ...
                write_json("light", last_change, 1)
            ...
            if time_delta > timedelta(minutes=2):
                ...
                write_json("light", last_change, 2)
